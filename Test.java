...
public class XMLReport {
	...
	// @ulyssesp Separated out ComplexityCalculator
	private final ComplexityCalculator complexity;
	private int indent = 0;

	// A lot going on in the constructor
	// Instead of passing in ProjectData could be a singleton class that takes in
	// ProjectData
	// It's really using the constructor as a single function. Java is weird
	public XMLReport(ProjectData projectData, File destinationDir, FileFinder finder, ComplexityCalculator complexity)
		...
		try {
			...
			// XML writer should be separate from the complexity stuff
			...
			println("<coverage line-rate=\"" + projectData.getLineCoverageRate() + "\" branch-rate=\""
					+ projectData.getBranchCoverageRate() + "\" lines-covered=\"" + numLinesCovered
					+ "\" lines-valid=\"" + numLinesValid + "\" branches-covered=\"" + numBranchesCovered
	...
	// dumpXYZ can be abstracted over an array
	// Most of it can be ablstracted over a walker pattern
	...
	private void dumpPackage(PackageData packageData) {
		...
		println("<package name=\"" + packageData.getName() + "\" line-rate=\"" + packageData.getLineCoverageRate()
				+ "\" branch-rate=\"" + packageData.getBranchCoverageRate() + "\" complexity=\""
	...
	private void dumpClass(ClassData classData) {
		...
		println("<class name=\"" + classData.getName() + "\" filename=\"" + classData.getSourceFileName()
				+ "\" line-rate=\"" + classData.getLineCoverageRate() + "\" branch-rate=\""
				+ classData.getBranchCoverageRate() + "\" complexity=\"" + complexity.getCCNForClass(classData) + "\""
				+ ">");
		increaseIndentation();
	...
	private void dumpMethod(ClassData classData, String nameAndSig) {
		...
		println("<method name=\"" + xmlEscape(name) + "\" signature=\"" + xmlEscape(signature) + "\" line-rate=\""
				+ lineRate + "\" branch-rate=\"" + branchRate + "\">");
		increaseIndentation();
